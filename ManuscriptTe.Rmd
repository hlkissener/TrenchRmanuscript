...
# APPENDIX S1
We provide an additional example using microclimate data measured along an elevation gradient in CO, USA to illustrate use of TrenchR. We first use a times series of air temperatures and wind speeds collected at multiple heights to examine profiles and surface roughness. We then use a time series of air and surface temperature, windspeed, and solar radiation collected at a single height to implement and test a biophysical model for grasshoppers.
...
We first examine how temperature and windspeed profiles can be used to scale conditions to the heights of organisms. We first use wind speed variation across hours and heights to estimate surface roughness.
```{r}
...
We then calculate the temperature and wind speed profiles for a given hour using multiple functions in TrenchR.
```{r}
...
The temperature and wind speed profiles illustrate how temperatures generally decrease and wind speeds generally increase with height above the ground. Profiles can be fit as a single smooth function or segments.
```{r, echo=FALSE}
...
We next illustrate estimating operative environmental temperatures, the estimated body temperatures for grasshoppers in the specified environments. We first read and process time series of environmental data. We estimate the zenith angle, the angle between the sun's rays and the vertical, across Julian days and hours.
```{r}
#Calculate zenith
...
for(ind in 1:nrow(dat) ){
  out= try(Tb_Gates(A=A, D=0.0211/3, psa_dir=0.66, psa_ref=1-0.66, psa_air=0.99, psa_g=0.01, T_g=dat$SoilTemp[ind]+273, T_a=dat$Tgrass[ind]+273, Qabs=A*dat$Rad[ind], epsilon=0.95, H_L=dat$H_L[ind], ef=1.3, K=0.15), silent=TRUE)
...
ggplot(data=dat.pl, aes(x=hour, y = value, color=variable))+geom_line(aes(lty=variable) )+facet_wrap(~elev.lab)+theme_classic()+ theme(legend.position="bottom")
