...
author: "Lauren B. Buckley, Bryan Briones Ortiz, Isaac Caruso, Aji John, Ofir Levy, Abigail Meyer, Yutaro Sakairi,..."
output:
...
John^1^, Ofir Levy^2^, Abigail Meyer^1^, Yutaro Sakairi^1^
...
averaged (*Abby: changed from aggregated*) environmental conditions. Yet, daily and seasonal
...
The TrenchR package aims to promote transparency and reproducibility.
...
(*Abby: maybe add the link to the reference website? trenchproject.github.io/TrenchR/*)
...
*conductive heat exchanges* between organisms and the ground due to physical
...
## Implementation
(*Abby: I might title this "Example Application" or "Demonstration of Use" or other. I would assume implementation to mean how you implemented the functions overall*)
...
(*Abby: I'm confused at where the distinction is made between Tb and Te, for example if the energy exchanges are part of Tb or Te or both.*)

Let us assume the lizard is in an unshaded location where a weather
station at standard height (2 meters) reports the daily air temperature
...
We first prepare the environmental data for analysis. We estimate 
hourly air and soil temperatures and radiation. We start by
...
weather station height (2 meters) to lizard height (0.02 meters). We assume a
surface roughness of $z_0 = 0.2$ (*Abby: units?*), which corresponds to bare sand and
...
A = sa_from_mass(mass, "lizard") # surface area (m^2)
psa = sapply(psi_deg, prop_silhouette_area, taxa = "lizard", posture = "prostrate") # Projected silhouette area (m^2)
...
```
*Abby: Added type=Gates above to fix error in H_L3, change if Gates is not the equation wanted. Also, maybe give a reason why one might want to use a spherical or simplified version*
