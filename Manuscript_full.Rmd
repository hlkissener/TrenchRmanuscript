...
1.  Much understanding of responses to climate change and variability relies on the
    ...
    organisms live near the ground and acute exposure to solar radiation and thermal extremes
...
2.  We introduce the TrenchR package, which aids in Translating
    ...
    radiation. We offer utility functions to aid in estimating the organismal and environonmental parameters needed for biophysical ecology.
3.  TrenchR focuses on relatively simple and modular functions so users can create transparent and flexible biophysical models.
4.  The package aims to introduce and enable microclimate and
    biophysical modelling to improve ecological and evolutionary
    forecasting. To further this aim, we offer a series of educational modules to introduce the field of biophysical ecology.
...
@kearney2009]. Applications are increasingly demonstrating the importance of accounting for microclimate variation [@bramer2018advances; @butikofer2020problem; @lembrechts2019comparing].  Acute thermal stress events are often more relevant to
...
introducing the TrenchR package (<https://github.com/trenchproject/TrenchR>) as a component of the TrEnCh project
(<https:www.trenchproject.com>), which builds computational and
...
[@kearney2017nichemapr] and biophysical [@kearney2020nichemapr] models. Although the Fortran source code was recently released, the complexity of the functions can make it difficult to understand and modify the source code. Many of the approaches are similar between the packages, but TrenchR focuses on
...
easy adaptation to a variety of organisms and research questions. Some
...
suited for ectothermic animals. We have limited inclusion of plant
...
We introduce the functions in several vignettes. A good place to start is
the *Allometry and conversions* vignette, which provides tools for
...
We will use the energy budget to estimate body temperature,
...
exchanges (omitting metabolism and evaporation, which are often negligible for ectotherms) can be estimated and often referred to as operative
...
We first prepare the environmental data for analysis. We will need
...
**Figure 1**. Body temperatures (Te) are predicted to drastically exceed air temperature when lizards are exposed to high levels of solar radiation. Air temperatures (Ta, °C) at lizard height (0.02m) are predicted to exceed air temperatures at 2m and to be similar to surface temperatures (Ts). We estimate body temperatures using two general energy budgets [solid: Tb_Gates();
dotted: Tb_CampbellNorman()] and a lizard specific biophysical model [dashed: Tb_lizard()] that differ in how they model heat exchanges.
...
[@kearney2017nichemapr; @kearney2020nichemapr]. We focus on models that predict equilibrium conditions for simplicity. Such models generally do not present computational challenges so our models are not optimized for computational efficiency. Making classic
...
evolutionary responses. Considering how organisms experience their environment is central to understanding responses to variable and changing environments [@suggitt2018extinction].
